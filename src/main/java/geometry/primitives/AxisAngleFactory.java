 ...
 *
 * Because AxisAngle4d cannot deal with singularities.
 *
 * Copied from (02/24/2018): http://www.euclideanspace.com/maths/geometry/rotations/conversions/matrixToAngle/
 */
public class AxisAngleFactory {

	private static double[][] convert(Matrix3d matrix) {
		matrix.normalize();
		double[][] m = new double[3][3];
		for (int x = 0; x < 3; x++) {
			for (int y = 0; y < 3; y++) {
				m[x][y] = matrix.getElement(x, y);
			}
		}
		return m;
	}

	public static AxisAngle toAxisAngle(Matrix3d matrix) {
		double[][] m = convert(matrix);
