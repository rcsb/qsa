 ...
 *
 */
public class LinearProgramChebyshev {
	
	private final Directories dirs = Directories.createDefault();
	private final Transformer transformer = new Transformer();
	
	private double dim = 2;
	private static final String eol = "\n";
	
	public void convert() throws Exception {
		int all = Integer.MAX_VALUE;
		
		Point3d[][] clustered = PointVectorDataset.read(dirs.getBiwordRepresentants(5.0), 3);
		...
		for (int x = 0; x < clustered.length; x++) {
			for (int y = 0; y < x; y++) {
				...
				String e = "a_" + equationNumber;
				String f = "b_" + equationNumber;
	...
	public static void main(String[] args) throws Exception {
		LinearProgramChebyshev lp = new LinearProgramChebyshev();
		lp.convert();
	}
}
